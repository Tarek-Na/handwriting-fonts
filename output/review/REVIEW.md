...
| 3 | Two stroke weights inside one exported font | +19/27/29%, t > 8 | **Medium-high**, visible in any word | (d) — the naive (b) fix was measured and rejected. **Worse after the framing fix** (writer 2: +29% → +48%): the model draws at font weight regardless of scale. Re-tested threshold matching at the new scale and rejected again for a structural reason — the strokes are near-binary, so level 0.5 → 0.8 moves the weight only 35% → 28%. Needs training. |
